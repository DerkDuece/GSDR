...
function Fire:startRegistered(registeredFireID, triggerDispatch, dispatchPlayer)
	...
	if self.registered[registeredFireID].dispatchCoords and triggerDispatch and dispatchPlayer then
		...
		Citizen.SetTimeout(
			Config.Fire.Dispatch.timeout,
			function()
				if Config.Fire.Dispatch.enabled then
...
function Fire:startSpawner(frequency, chance)
	frequency = tonumber(frequency) or Config.Fire.randomfire.interval
	chance = tonumber(chance) or Config.Fire.randomfire.chance
	...
	Citizen.CreateThread(
		function()
			while spawnerActive do
				if next(self.random) and not self.currentRandom and Dispatch:firefighters() >= Config.Fire.randomfire.players then
