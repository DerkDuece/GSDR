...
function onResourceStart(resourceName)
	if (GetCurrentResourceName() == resourceName) then
		Whitelist:load()
		Fire:loadRegistered()
		if Config.Fire.randomfire.enableRandomFire and Config.Fire.randomfire.interval then
...
AddEventHandler('fireManager:command:startfire', function(coords, maxSpread, chance, triggerDispatch, dispatchMessage)
	...
	if triggerDispatch then
		Citizen.SetTimeout(Config.Fire.Dispatch.timeout, function()
			if Config.Fire.Dispatch.enabled and not Config.Dispatch.disableCalls then
...
if Config.Fire.Dispatch.enabled then
    if Config.Framework == "esx" then
        local allowedJobs = {}
		local firefighterJobs = Config.Fire.randomfire.firefighterJobs or {}
