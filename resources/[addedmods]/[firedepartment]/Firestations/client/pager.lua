...
Pager.WaitTime = wk.WaitTime
-- List of tones that can be paged
Pager.Tones = wk.Tones
...
FireSiren.Stations = wk.Stations
	...
	if PlayerJob or PlayerJob.name == Config.JobName then
		function EnablePager()
			...
			if not #Args ~= #Pager.TunedTo and #Args ~= 0 then
				...
				for _, Tone in ipairs(Pager.TunedTo) do
					QBCore.Functions.Notify(Lang:t('success.pagerset') .. Tone:upper(), 'info')
				end

				QBCore.Functions.Notify(Lang:t('success.pagerset'), false, 'success')
				Pager.Enabled = true
			else
				QBCore.Functions.Notify(Lang:t('error.Invalid_tones'), true, 'error')
				Pager.Enabled = false
				Pager.TunedTo = {}
			end
		end
		...
		if not Pager.Enabled then
			...
			if #Args ~= 0 then
				...
				QBCore.Functions.Notify(Lang:t('error.turned_off'), false, 'error')
				Pager.Enabled = false
				Pager.TunedTo = {}
			end
		end
		...
		QBCore.Functions.Notify(Lang:t('error.whitelisted'), true, 'error')
	end
end)
...
RegisterCommand("firesiren", function(Source, Args)
	...
	if PlayerJob or PlayerJob.name == Config.JobName then
		...
		QBCore.Functions.Notify(Lang:t('error.whitelisted'), "error")	
	end
end)
		...
		if not Pager.Paging then
			for _, ProvidedTone in ipairs(Args) do
				for _, ValidTone in ipairs(Pager.Tones) do
					...
					elseif ProvidedTone:lower() == wk.PageSep then
						HasDetails = true
			...
			if not ToneCount ~= #ToBeCanceled and ToneCount ~= 0 then
				for _, Tone in ipairs(ToBeCanceled) do
					QBCore.Functions.Notify(Lang:t('info.Cancel') .. Tone:upper(), 'info')
				end
					QBCore.Functions.Notify(Lang:t('info.Cancel'), false, 'info')
			...
			QBCore.Functions.Notify(Lang:t('error.examined'), true, 'error')
		end
	-- If player is not whitelisted
	else
		QBCore.Functions.Notify(Lang:t('error.whitelisted'), true, 'error')
	...
	if Pager.Enabled then
		...
		if NeedToPlay then
			...
			for _, Tone in ipairs(Tones) do
				...
				QBCore.Functions.Notify(Lang:t('info.Reports') .. Tone:upper(), 'info')
				else
			...
			if HasDetails then
				...
				TriggerEvent("chat:addMessage", {
					...
					color = { 255, 0, 0},
					multiline = true,
					args = {"Fire control", "\nAttention " .. wk.DeptName .. " - " .. NewDetails .. " - " .. NewTones .. "Emergency.\n\nTime out " .. Hours .. Minutes.. "."}
				})
			else
				TriggerEvent("chat:addMessage", {
					templateId = "page",
					...
					color = { 255, 0, 0},
					multiline = true,
					args = {"Fire control", "\nAttention " .. wk.DeptName .. " - " .. wk.DefaultDetails .. ".\n\nTime out " .. Hours .. Minutes.. "."}
				})
			end
		else
...
AddEventHandler("qb-Firestations:CancelPage", function(Tones, HasDetails, Details)
	...
	if Pager.Enabled then
		...
		if NeedToPlay then
			QBCore.Functions.Notify(Lang:t('success.turned_on'), true, 'success')
			Citizen.Wait(1500)
			...
			if HasDetails then
				...
				TriggerEvent("chat:addMessage", {
					...
					color = { 255, 0, 0},
					multiline = true,
					args = {"Fire control", "\nAttention " .. wk.DeptName .. " - Call canceled, ignore the answer - " .. NewDetails}
				})
			-- If no details provided
				...
				TriggerEvent("chat:addMessage", {
					...
					color = { 255, 0, 0},
					multiline = true,
					args = {"Fire control", "\nAttention " .. wk.DeptName .. " - Call canceled, ignore the answer."}
				})
			end
		else
			Citizen.Wait(1500)
        ...
        if not Pager.Paging then
            ...
            if not ToneCount ~= #ToBePaged and ToneCount ~= 0 then
                for _, Tone in ipairs(ToBePaged) do
					QBCore.Functions.Notify(Lang:t('info.search').. Tone:upper(), 'info')
                end
				QBCore.Functions.Notify(Lang:t('info.search'), false, 'info')
			...
			QBCore.Functions.Notify(Lang:t('info.searched'), true, 'info')
        end
    -- If player is not whitelisted
    else
		QBCore.Functions.Notify(Lang:t('error.whitelisted'), true, 'error')
...
function SoundFireSiren(Args)
		...
		for _, ValidStation in ipairs(FireSiren.Stations) do
			if ProvidedStation:lower() == ValidStation.Name then
				if not FireSiren.EnabledStations[ProvidedStation:lower()] then
					...
					QBCore.Functions.Notify(Lang:t('error.supplied'), 'error')
					return
				end
			end
		end
	...
	if not #Args ~= #ToBeSirened and #Args ~= 0 then
		...
		QBCore.Functions.Notify(Lang:t('error.Sound_Sirens'), 'error')
	end
end
