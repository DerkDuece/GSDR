...
for i, coord in ipairs(Config.Muscleweight) do
    exports.ox_target:addSphereZone("muscleweight" .. i, vec3(coord.x, coord.y, coord.z), 0.95, {
        useZ = true,
        name = "muscleweight",
        debugPoly = false
    }, {
        options = {
            {
                action = function()
                    QBCore.Functions.TriggerCallback('krp_gym:checkitem', function(ano)
                        if ano >= 1 then
                            QBCore.Functions.TriggerCallback('krp_gym:checkCooldown', function(cooldownRemaining)
                                if cooldownRemaining <= 0 then
                                    ...
                                    if muscleweight then
                                        ...
                                        DeleteEntity(prop)
                                        Wait(2000)
                                        QBCore.Functions.TriggerCallback('getStrength', function(strength)
                                            if strength then
