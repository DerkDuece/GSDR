...
for i, v in ipairs(Config.Coordonate) do
    ...
    exports.ox_target:addSphereZone("SHOP", vector3(x, y, z+1), 0.75, {
        name = "SHOP",
        debugPoly = false,
    }, {
        options = {
...
AddEventHandler('krp_shop:buying', function()
    ...
    QBCore.Functions.TriggerCallback("krp_shop:checkMoney", function(hasEnoughMoney)
        if hasEnoughMoney then
            ...
            QBCore.Functions.TriggerCallback('krp_shop:buy', function(success)
                if success then
...
AddEventHandler('krp_rent_dialog', function()
    ...
    QBCore.Functions.TriggerCallback('bike_shop:buyBike', function(bought)
        if bought then
